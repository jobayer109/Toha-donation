"use client"

import { useState, useEffect } from "react"

export function CountdownTimer({ targetDate }) {
  const [timeLeft, setTimeLeft] = useState({
    days: 0,
    hours: 0,
    minutes: 0,
    seconds: 0,
  })

  useEffect(() => {
    const calculateTimeLeft = () => {
      const difference = new Date(targetDate) - new Date()

      if (difference > 0) {
        setTimeLeft({
          days: Math.floor(difference / (1000 * 60 * 60 * 24)),
          hours: Math.floor((difference / (1000 * 60 * 60)) % 24),
          minutes: Math.floor((difference / 1000 / 60) % 60),
          seconds: Math.floor((difference / 1000) % 60),
        })
      } else {
        setTimeLeft({ days: 0, hours: 0, minutes: 0, seconds: 0 })
      }
    }

    calculateTimeLeft()
    const timer = setInterval(calculateTimeLeft, 1000)

    return () => clearInterval(timer)
  }, [targetDate])

  return (
    <div className="flex items-center gap-2 md:gap-3 font-mono">
      <div className="flex flex-col items-center">
        <span className="text-lg md:text-xl font-bold text-amber-800">{timeLeft.days}</span>
        <span className="text-xs text-amber-700">days</span>
      </div>
      <span className="text-amber-800 font-bold">:</span>
      <div className="flex flex-col items-center">
        <span className="text-lg md:text-xl font-bold text-amber-800">
          {timeLeft.hours.toString().padStart(2, "0")}
        </span>
        <span className="text-xs text-amber-700">hours</span>
      </div>
      <span className="text-amber-800 font-bold">:</span>
      <div className="flex flex-col items-center">
        <span className="text-lg md:text-xl font-bold text-amber-800">
          {timeLeft.minutes.toString().padStart(2, "0")}
        </span>
        <span className="text-xs text-amber-700">mins</span>
      </div>
      <span className="text-amber-800 font-bold">:</span>
      <div className="flex flex-col items-center">
        <span className="text-lg md:text-xl font-bold text-amber-800">
          {timeLeft.seconds.toString().padStart(2, "0")}
        </span>
        <span className="text-xs text-amber-700">secs</span>
      </div>
    </div>
  )
}

